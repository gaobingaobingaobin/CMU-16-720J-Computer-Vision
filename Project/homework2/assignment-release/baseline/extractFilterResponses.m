...
%Convert to Lab
doubleI = double(I); %uint8 to double
[L,a,b] = RGB2Lab(doubleI(:,:,1), doubleI(:,:,2), doubleI(:,:,3)); %RGB to Lab
pixelCount = size(doubleI,1)*size(doubleI,2); %number of total pixels
filterResponses = zeros(pixelCount, length(filterBank)*3);%matrix of pixelCount*length(filterBank)*3)
